...
#   /home/it-admin/esedu-tiketti/deploy.sh
#
# Tai omalta koneelta yhdellä komennolla:
#   ssh it-admin@tiketti.esedu.fi '/home/it-admin/esedu-tiketti/deploy.sh'
#
...
main() {
  ...
  echo
  echo "✅ Julkaisu valmis - $(git -C "$REPO_DIR" rev-parse --short HEAD) ($(git -C "$REPO_DIR" log -1 --format=%s))"
  echo "   Rollback: cd $REPO_DIR && git checkout $PREV_SHA && SKIP_GIT=1 ./deploy.sh"
}
...
preflight() {
  for cmd in git docker rsync curl; do
...
health_check() {
  ...
  echo >&2
  echo "Rollback: cd $REPO_DIR && git checkout $PREV_SHA && SKIP_GIT=1 ./deploy.sh" >&2
  exit 1
}
